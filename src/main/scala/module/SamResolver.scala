...
class SamResolver extends IdResolver {
  ...
  val tileMap: scala.collection.Map[Tile, IdTile] = {
    ...
    for ((sam, offset) <- samOffsets.drop(1)) {
      ...
      add(sam~NS & Tla5~EW, 0x5e646000 + offset)                // TLA-5 +
      ...
      add(sam~CS & Tla3~WE, 0x5e650000 + offset)  // Tla3 Thru
      add(sam~NS & Tla3~CE, 0x5e660000 + offset)  // Tla3 Ends
      ...
      add(sam~CN & Tla5~EW, 0x5e656000 + offset)    // Tla5 short T
